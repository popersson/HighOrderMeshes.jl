...
"""
    facemap(::Block{D}) where {D}

Generated by extrusion from `facemap(Block{D-1})`: each face of `Block{D-1}`
is extended into a face of `Block{D}` by adding the two layers `ξ_D = 0, 1`,
and the two new faces perpendicular to `ξ_D` are built from scratch by
[`_oriented_block_face`](@ref). A face's vertex list is oriented so that its
outward normal followed by its own local axes is right-handed; extruding a
correctly oriented `(D-1)`-face keeps that property except where there was no
axis available to enforce it (the point-faces of `Block{1}`), so the extruded
layers are swapped there when the orientation determinant comes out negative.
"""
function facemap(::Block{D}) where {D}
    ...
    nvl   = 2^(D-1)
    vlow  = vertices(Block{D-1}())
    extruded = similar(lower, 2 * size(lower, 1), size(lower, 2))
    for i in axes(lower, 2)
        d, side = (i + 1) ÷ 2, (i - 1) % 2     # column i is face d,side of Block{D-1}
        col  = lower[:, i]
        n    = zeros(Int, D - 1); n[d] = side == 0 ? -1 : 1
        vf   = vlow[col, :]
        naxes = Int(log2(length(col)))                    # tensor-ordered face has 2^naxes vertices
        axes_ = [(vf[1 + 2^(k-1), :] .- vf[1, :])' for k in 1:naxes]
        A    = vcat(n', axes_...)
        extruded[:, i] = det(A) < 0 ? vcat(col .+ nvl, col) : vcat(col, col .+ nvl)
    end
...
# The face ξ_D = `side` of Block{D}, in the tensor order of its Block{D-1}
# vertex list, flipping the last local axis (the outer half of the list) when
# the outward normal followed by the local axes would otherwise be
# left-handed.
function _oriented_block_face(D, side)
    nvl   = 2^(D - 1)
    verts = collect(1:nvl)
    n = zeros(Int, D); n[D] = side == 0 ? -1 : 1
    A = zeros(Int, D, D)
    A[1, :] = n
    for k in 1:D-1
        A[k+1, k] = 1
    ...
    if det(A) < 0
        half = nvl ÷ 2
        verts = vcat(verts[half+1:end], verts[1:half])
    end
    verts .+ side * nvl
end
...
edgemap(eg::ElementGeometry{2}) = facemap(eg)      # in 2D edges are faces: Simplex{2} and Block{2}
edgemap(::Block{1}) = reshape([1, 2], 2, 1)
edgemap(::Block{2}) = facemap(Block{2}())          # disambiguates ElementGeometry{2} vs Block{D}

"""
    edgemap(::Block{D}) where {D >= 3}

Edges of `Block{D}`, grouped by direction `d = 1, ..., D`. For each direction,
edges are listed for every position of the other `D-1` coordinates in
lexicographic order (the lowest-numbered coordinate varying fastest), oriented
from `ξ_d = 0` to `ξ_d = 1`.
"""
function edgemap(::Block{D}) where {D}
    v = vertices(Block{D}())
    idx = Dict(Tuple(v[k, :]) => k for k in axes(v, 1))
    cols = Vector{Vector{Int}}()
    for d in 1:D, bits in Iterators.product(ntuple(_ -> 0:1, D - 1)...)
        other = setdiff(1:D, d)
        c = zeros(Int, D)
        for (j, k) in enumerate(other)
            c[k] = bits[j]
        end
        c[d] = 0; v0 = idx[Tuple(c)]
        c[d] = 1; v1 = idx[Tuple(c)]
        push!(cols, [v0, v1])
    end
    reduce(hcat, cols)
end
